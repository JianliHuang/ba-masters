setwd("/home/navarurh/documents/subjects/businessanalytics-R/lec1/")
######################################
## Author:   Jason Parker
## Date:     2018-08-20
## Title:    lec1.R
...
#change when no cig only cigsq
modeltest         <- lm(bwght~I(cigs^2), data=bwght) #this does work because R treats statements inside of the I() as literal
summary(modeltest)
plot_quad_test      <- plot_ols + geom_line(aes(y=predict(modeltest)),color="blue",size=2)
plot_quad_test
#curves down as it is a variation on how we are creating the equation
