...
## parameters
nx        <- c(250,500,1000)    # number of replications per digit (small for fast test)
...
#running the scenarios for the different sample sizes based on subsetting the data in simDigits
for (x in c(0:2))
{
# setup: 10x logit
r1 <- if(length(10*nx[x])==0){0}else(10*nx[x]) #creating the start rownum for sample subset
r2 <- 10*nx[x+1]                            #this is the end rownum for the sample subset 
td            <- simDigits[r1:r2,]  #creating an appropriate sized sample for this case
