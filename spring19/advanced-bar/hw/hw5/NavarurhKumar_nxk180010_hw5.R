...
library(data.table)
raw <- read.table('ba-masters/spring19/advanced-bar/hw/hw5/Concrete_Data_wj.csv',header = T,sep = ',')
...
idx_cv <- test$idx
resid_cv <- test$prediction - test$strength
...
for(i in 1:1030)
{
  m <- lm(strength~.,data = raw[raw$idx!=i,1:9])
  ...
  resid_jk <- pred - raw$strength[raw$idx==i]
  idx_jk <- i
...
for(i in 1:10)
{
  ...
  resid_cv <- test$predict - test$strength
  idx_cv <- test$idx
  grp_cv <- i
